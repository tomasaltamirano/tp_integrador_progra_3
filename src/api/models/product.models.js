...
const insertProducto = (nombre, precio, descripcion, imagen, categoria) => {
    const sql = 'INSERT INTO productos (nombre, precio, descripcion, imagen, categoria, activo) VALUES (?, ?, ?, ?, ?, 1)';
    return connection.query(sql, [nombre, precio, descripcion, imagen, categoria]);
};

const updateProducto = (nombre, precio, descripcion, imagen, categoria, id) => {
    const sql = 'UPDATE productos SET nombre = ?, precio = ?, descripcion = ?, imagen = ?, categoria = ? WHERE id = ?';
    return connection.query(sql, [nombre, precio, descripcion, imagen, categoria, id]);
};
